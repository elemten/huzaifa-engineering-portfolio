...
export const caseStudies: CaseStudy[] = [
  {
    ...
    metrics: [
      { value: "288", label: "documents made searchable", detail: "Legacy files moved into a filterable public library instead of scattered page links." },
      { value: "7→1", label: "domains under one model", detail: "Services, documents, news, jobs, communities, contacts, and departments share a publishing structure." },
      { value: "6", label: "communities represented", detail: "One maintainable system supports distinct community information and service entry points." },
    ],
    architecture: [
  ...
  },
  {
    ...
    metrics: [
      { value: "5", label: "operations brought together", detail: "Memberships, bookings, finance, events, and schedules use shared operational data." },
      { value: "3", label: "manual handoffs automated", detail: "Payment events, invoice generation, and calendar synchronization run server-side." },
      { value: "1", label: "role-aware operating view", detail: "Public journeys and staff workflows live in one governed platform instead of disconnected tools." },
    ],
    architecture: [
  ...
  },
  {
    ...
    metrics: [
      { value: "1", label: "factual source of truth", detail: "The candidate maintains one history instead of re-entering claims for every application." },
      { value: "6", label: "workflow stages automated", detail: "Ingestion, job analysis, grounding, generation, validation, and rendering run as a repeatable pipeline." },
      { value: "2", label: "ready-to-use outputs", detail: "The same grounded workflow produces a tailored résumé or cover letter PDF." },
    ],
    architecture: [
  ...
  },
  {
    ...
    repoUrl: "https://github.com/elemten/gis-road-matcher",
    repoLabel: "Sanitized public reference implementation",
    heroImage: "",
    ...
    metrics: [
      { value: "4", label: "lookup steps joined", detail: "Capture, validation, spatial matching, and visual review run in one workflow." },
      { value: "1", label: "review surface", detail: "Raw point, snapped point, distance, and road geometry stay visible together." },
      { value: "5km", label: "explicit match boundary", detail: "The public reference preserves no-result instead of assigning an unrelated distant road." },
    ],
    architecture: [
  ...
  },
  {
    slug: "autojobapply",
    index: "06",
    title: "ApplyPilot Control Panel",
    shortTitle: "ApplyPilot Control Panel",
    eyebrow: "Human-in-the-loop automation · 2026",
    summary:
      "A deployed control surface for discovering, enriching, scoring, and preparing role-specific application material while keeping final submission with the candidate.",
    cardSummary:
      "Made a multi-stage job workflow visible as a queue, with stage-level status, explicit human approval, and a responsive control surface.",
    role: "Adapter, integrator & control-panel developer",
    period: "Jul 2026",
    industry: "Career workflow automation",
    status: "Deployed demo",
    repoLabel: "Derivative of ApplyPilot (AGPL-3.0)",
    heroImage: "/images/case-studies/applypilot-control-panel.png",
    heroAlt: "ApplyPilot deployed control panel showing four inspectable preparation stages.",
    accent: "#2563eb",
    tags: ["React", "Python", "OpenAI", "Vercel", "Playwright"],
    challenge:
      "The upstream ApplyPilot workflow covered broad job automation, but an autonomous command-line pipeline is difficult to supervise. The adaptation needed a clear OpenAI/Codex path and a control surface where a candidate can inspect preparation stages without handing submission authority to the browser.",
    constraints: [
      ...
      "Expose stage state and failure honestly, including mock mode and missing backend configuration.",
    ],
    contribution: [
      "Designed and deployed a responsive control panel for discovery, enrichment, scoring, and document preparation.",
      "Made each stage manually startable with automatic status polling and stage-level counts.",
      "Kept application submission outside the control panel and made that boundary visible in the interface.",
      "Kept the upstream project and original author prominent in repository documentation and product positioning.",
    ],
    metrics: [
      { value: "4", label: "visible queue stages", detail: "Discovery, enrichment, scoring, and document preparation have separate status and controls." },
      { value: "0", label: "automatic submissions", detail: "The deployed control panel prepares work but leaves the final application with the candidate." },
      { value: "AGPL", label: "provenance preserved", detail: "ApplyPilot and Pickle-Pixel remain explicitly credited as the upstream project and author." },
    ],
    architecture: [
      ...
      { label: "Document pipeline", detail: "Grounded résumé and cover-letter preparation" },
      { label: "Human handoff", detail: "Direct application link and candidate-controlled submission" },
    ],
    decisions: [
      { title: "Make the queue the product", detail: "Stage status, counts, and controls turn a long-running workflow into something a candidate can inspect and supervise." },
      { title: "Keep submission human-controlled", detail: "The panel prepares the résumé, cover letter, and direct link, but does not fill or submit applications." },
    ...
    security: [
      ...
      "Pipeline stages can be reviewed before any external application action.",
      "CAPTCHA-blocked applications fail visibly rather than bypassing controls silently.",
    ],
    screenshots: [
      { src: "/images/case-studies/applypilot-control-panel.png", alt: "ApplyPilot desktop control panel in idle demo mode.", caption: "The deployed desktop surface exposes four preparation stages and the human submission boundary." },
      { src: "/images/case-studies/applypilot-running.png", alt: "ApplyPilot control panel with the discovery stage running.", caption: "Starting discovery changes the queue to a visible running state with status polling." },
      { src: "/images/case-studies/applypilot-mobile.png", alt: "ApplyPilot control panel on a mobile viewport.", caption: "The same control and trust boundaries remain readable on a narrow viewport." },
    ],
    evidence: [
      "Vercel deployment verified in idle, running, and mobile states",
      "Control panel labels demo mode and missing API configuration instead of masking degraded state",
      "README preserves ApplyPilot and Pickle-Pixel attribution",
      "AGPL-3.0 license and upstream package name remain intact",
      "Python pipeline and deployed control surface are presented as an adaptation, not ground-up authorship",
    ],
    reflection:
      "The strongest change was not adding more autonomy. It was exposing the workflow and returning submission authority to the candidate. The remaining production step is connecting the deployed panel to its API; until then, the portfolio labels it as a demo rather than overstating readiness.",
  },
  {
    slug: "quest-disposal",
    index: "07",
